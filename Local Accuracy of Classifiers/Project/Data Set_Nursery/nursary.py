...
df = pd.read_csv("nursary.data", header=None, names=headers, na_values="?")
replace_lebels = {"class": {"not_recom": 0, "recommend": 0, "priority":0, "spec_prior":0, "very_recom":1}}
df.replace(replace_lebels, inplace=True)

new_df = pd.DataFrame(data=pd.get_dummies(df, columns=["parents", "has_nurs", "form", "children", "housing",
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.50, random_state=36851234)
splits=2
repeats=2
