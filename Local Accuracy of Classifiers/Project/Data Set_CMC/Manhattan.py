...
max_value = df[["wife_age","no_child"]].max()

min_value = df[["wife_age","no_child"]].min()

std = df[["wife_age","no_child"]].std()

range= max_value-min_value
df[["wife_age","no_child"]]= \
    ((df[["wife_age","no_child"]]-min_value)/std)
...
for index in minority_index:
    distances=[]
    for b in X:
        distances.append(Manhattan(X[index],b))

    sorted_k_distances=np.argsort(distances)[0:10]
    ...
    if (minority_count == 9 and majority_count == 0) or (minority_count == 8 and majority_count == 1) or (minority_count == 7 and majority_count == 2) or (minority_count == 6 and majority_count == 3):
        # Append the safe and predict data to list
        safe = safe + 1

    elif (minority_count == 5 and majority_count == 4) or (minority_count == 4 and majority_count == 5):
        # Append the boarderline and predict data to list
        boarderline = boarderline + 1

    elif (minority_count == 3 and majority_count == 6) or (minority_count == 2 and majority_count == 7) or (minority_count == 1 and majority_count == 8):
